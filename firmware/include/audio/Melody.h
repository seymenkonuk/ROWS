...
// CLASSES
class Melody {
  static constexpr uint32_t MAX_MELODY_SIZE = 1000;

public:
  void setTempo(uint8_t value);
  bool setNote(uint8_t frequencyId, uint8_t durationId);
...
private:
  bool setFrequency(uint8_t value, uint32_t index);
  bool setDuration(uint8_t value, uint32_t index);

  uint8_t tempo;
  uint8_t frequency[MAX_MELODY_SIZE];
  uint8_t durations[MAX_MELODY_SIZE];

  uint32_t noteCount = 0;
  uint32_t currentIndex = 0;

  static uint32_t notesFrequency[];
