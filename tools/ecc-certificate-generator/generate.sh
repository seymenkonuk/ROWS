...
if [ "$1" = "reset" ]; then
  ...
  openssl req -new -x509 -days 3650 -key ca.key -config ../../utils/ecc-certificate-generator/cnf/root.cnf -out ca.crt

  # 2. Server Sertifikası
  echo "Creating Server Certificate..."
  openssl ecparam -name prime256v1 -genkey -noout -out server.key
  openssl req -new -key server.key -config ../../utils/ecc-certificate-generator/cnf/server.cnf -out server.csr
  openssl x509 -req -in server.csr -CA ca.crt -CAkey ca.key -CAcreateserial -extfile ../../utils/ecc-certificate-generator/cnf/server.cnf -extensions v3_req -out server.crt -days 3650
  rm server.csr
fi
...
openssl req -new -key $CLIENT_NAME.key -config ../../utils/ecc-certificate-generator/cnf/client.cnf -out client.csr
openssl x509 -req -in client.csr -CA ca.crt -CAkey ca.key -CAcreateserial -extfile ../../utils/ecc-certificate-generator/cnf/client.cnf -extensions v3_client -out $CLIENT_NAME.crt -days 3650
rm client.csr
